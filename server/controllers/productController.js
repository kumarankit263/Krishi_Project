...
// Get Seller Dashboard Data //done
const getProductDataBySellerId = async (req, res) => {
  try {
    // Get the sellerId from the query parameters
    const sellerId = req.query.sellerId;

    // If sellerId is not provided, return a 400 error
    if (!sellerId) {
    ...
    if (data.length === 0) {
      return res.status(404).send({ message: "No products found for this seller" });
    }
...
const getMainProductDataById = async (req, res) => {   //done
  try {
    ...
    const product = await Product.findById(new mongoose.Types.ObjectId(productId))
      .select("name image brand measuringUnit pricePerUnit minimumOrderQuantity location.coordinates sellerId")
      .lean();
