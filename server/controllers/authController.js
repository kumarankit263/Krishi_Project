...
// const bcrypt = require("bcryptjs");
// const { authModelSelector, generateAccessToken } = require("../services/authServices");
// const { setCookie } = require("../services/cookieServices");
...
// //       if (error.keyPattern.email || error.keyPattern.contact) {
// //         return res.status(400).send({
// //           message: `${capitalizeFirstLetter(type)} with this email or phone number already exists`,
// //         });
// //       } else if (type === "seller" && error.keyPattern.brandName) {
...
//     if (!data) {
//       return res.status(404).send({ message: `${capitalizeFirstLetter(type)} not found` });
//     }
...
//       if (!data.isVerified) {
//         return res.status(400).send({
//           message: `${capitalizeFirstLetter(type)} account not verified`,
//         });
//       }
...
// //     let data = await Model.findOne({ email }).select(
// //       `password ${type === "seller" && "brandName"}`
// //     );
...
//     const data = await Model.findOne({ verificationToken });
...
//     // No account verification logic here
//     return res.status(200).send({
